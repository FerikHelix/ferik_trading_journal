  ...
  return <>
    ...
    <section className="card">
      <div className="toolbar">
        <input aria-label="Cari trade" placeholder="Cari symbol, ticket, strategy, tag…" value={query} onInput={e=>setQuery(e.currentTarget.value)}/>
        <select aria-label="Filter arah" value={side} onChange={e=>setSide(e.currentTarget.value)}><option value="">Buy & sell</option><option value="buy">Buy</option><option value="sell">Sell</option></select>
        <select aria-label="Filter hasil" value={result} onChange={e=>setResult(e.currentTarget.value)}><option value="">Semua hasil</option><option value="win">Win</option><option value="loss">Loss</option><option value="be">Breakeven</option></select>
        <select aria-label="Filter strategy" value={strategy} onChange={e=>setStrategy(e.currentTarget.value)}><option value="">Semua strategy</option>{strategies.map(value=><option key={value}>{value}</option>)}</select>
        <select aria-label="Filter journal" value={journalStatus} onChange={e=>setJournalStatus(e.currentTarget.value)}><option value="">Semua journal</option><option value="complete">Sudah diisi</option><option value="empty">Belum diisi</option></select>
        <input aria-label="Tanggal mulai" type="date" value={from} onInput={e=>setFrom(e.currentTarget.value)}/><input aria-label="Tanggal akhir" type="date" value={to} onInput={e=>setTo(e.currentTarget.value)}/>
        <select aria-label="Urutkan" value={sort} onChange={e=>setSort(e.currentTarget.value)}><option value="date-desc">Terbaru</option><option value="date-asc">Terlama</option><option value="pnl-desc">P&amp;L terbesar</option><option value="pnl-asc">P&amp;L terkecil</option><option value="volume-desc">Volume terbesar</option></select>
      </div>
