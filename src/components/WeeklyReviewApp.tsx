...
export default function WeeklyReviewApp() {
  ...
  const [accountId, setAccountId] = useState(''); const [bounds, setBounds] = useState(() => weekBounds()); const [review, setReview] = useState<WeeklyReview>();
  ...
  return <div className="grid review-page">
    <section className="card review-controls"><div><span className="eyebrow">Weekly review</span><h2>{weekLabel(bounds.weekStart, bounds.weekEnd)}</h2><p className="muted">Senin–Minggu · UTC</p></div><div className="review-actions"><div className="field"><label htmlFor="review-account">Account</label><select id="review-account" value={accountId} onChange={(event) => setAccountId(event.target.value)}>{accounts.map((item) => <option key={item.id} value={item.id}>{item.label} · {item.currency}</option>)}</select></div><div className="actions"><button className="btn" aria-label="Minggu sebelumnya" onClick={() => setBounds(shiftWeek(bounds.weekStart, -1))}>←</button><button className="btn" onClick={() => setBounds(weekBounds())}>Minggu ini</button><button className="btn" aria-label="Minggu berikutnya" onClick={() => setBounds(shiftWeek(bounds.weekStart, 1))}>→</button></div></div></section>
    ...
    {review && <section className="card review-reflection"><div className="card-head"><div><h2>Refleksi mingguan</h2><p className="muted">Catatan ini tersimpan lokal untuk account dan minggu ini.</p></div><span className={`save-state ${saveState === 'error' ? 'loss' : ''}`}>{saveState === 'saving' ? 'Menyimpan…' : saveState === 'saved' ? '✓ Tersimpan' : saveState === 'error' ? 'Gagal menyimpan' : 'Autosave aktif'}</span></div><div className="form-grid"><label className="field"><span>Yang berjalan baik</span><textarea value={review.wentWell} onChange={(event) => update('wentWell', event.target.value)} placeholder="Apa yang kamu lakukan dengan baik?" /></label><label className="field"><span>Hal yang perlu diperbaiki</span><textarea value={review.toImprove} onChange={(event) => update('toImprove', event.target.value)} placeholder="Pola atau keputusan apa yang perlu diperbaiki?" /></label><label className="field"><span>Lesson minggu ini</span><textarea value={review.lesson} onChange={(event) => update('lesson', event.target.value)} placeholder="Pelajaran paling penting dari minggu ini" /></label><label className="field"><span>Fokus minggu depan</span><textarea value={review.nextWeekFocus} onChange={(event) => update('nextWeekFocus', event.target.value)} placeholder="Satu atau dua fokus yang spesifik" /></label></div></section>}
  </div>;
}
